...
const loadRelationTypes = async () => {
  ...
  try {
    const res = await kgRelationType.listByProjectId(projectId.value)
    if (res.code === 200) {
...
const handleAddEntityType = () => {
  ...
  entityForm.id = null
  entityForm.projectId = projectId.value
  entityForm.name = ''
...
const handleAddRelationType = () => {
  ...
  relationForm.projectId = projectId.value
  relationForm.name = ''
